...
from utils.ExecutionContext import (
    TestRunInfo,
    get_context,
    get_timeout,
    has_bonus,
...
class BaseTester:
    ...
    def test_selector(self):
        ...
        if not selected_testers:
            ...
            for i, tester in enumerate(self.testers):
                print(
                    f"{TC.B_BLUE}    {i + 1}) {TC.B_WHITE}{tester.name}{TC.NC} ({tester.git_url})"
                )
            print(
                f"You can pass the numbers as arguments to {TC.B_WHITE}--testers{TC.NC} to not see this prompt"
            )
    ...
    def prepare_ex_files(self):
        if os.path.exists(self.temp_dir):
            logger.info(f"Removing existing directory {self.temp_dir}")
            shutil.rmtree(self.temp_dir)

        logger.info(
            f"Copying {self.source_dir} to {self.temp_dir} while ignoring git-ignored files"
        )

        repo = git.Repo(self.source_dir)
        tracked_files = set(repo.git.ls_files().split("\n"))

        def ignore_untracked_files(dir, files):
            # Compute relative paths
            dir_rel_path = os.path.relpath(dir, self.source_dir)
            ignored = []
            for f in files:
                rel_path = os.path.normpath(os.path.join(dir_rel_path, f))
                if rel_path not in tracked_files and f != ".git":
                    ignored.append(f)
            return ignored

        shutil.copytree(self.source_dir, self.temp_dir, ignore=ignore_untracked_files)
    ...
    def prepare_tests(self, tester):
        ...
        if os.path.exists(temp_dir):
            logger.info(f"Removing existing directory {temp_dir}")
    ...
    def show_summary(self, norm: str, missing, errors, to_execute):
        ...
        if not has_errors:
            ...
            print(
                f"{TC.CYAN}║                 🎉🥳 {TC.B_GREEN}All tests passed! Congratulations!{TC.CYAN} 🥳🎉                 ║"
            )
            print(
                f"{TC.CYAN}╚══════════════════════════════════════════════════════════════════════════════╝"
            )
        ...
        print(f"\n{TC.B_CYAN}Summary{TC.NC}: {'bonus' if has_bonus() else 'no bonus'}")

        logger.warning(f"Norminette errors: {norm_errors}")
        if norm_errors:
        ...
        logger.warning(f"Errors in functions: {errors}")
        if error_funcs:
